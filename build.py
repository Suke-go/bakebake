...
ACT_SLUGS = ["yokai-expo-2026", "jsda-2026", "shonan-shirayuri", "xr-kaigi", "xr-meetup-aichi"]
...
def shell(lang, page, body, root, home=False, title=None):
...
<title>{ttl}</title>
<meta name="description" content="{t['title']} | {t['full']}">
...
def page_activities(lang):
    ...
    for i, (label, date, image, title, text, link) in enumerate(t["acts"]):
        ...
        items.append(f'<li class="act"><a href="activities/{ACT_SLUGS[i]}.html">{im}<div><span class="lab">{label}</span><time>{date}</time><b>{title}</b></div></a></li>')
...
<section class="page detail">
  <div class="wrap">
    ...
    <div class="tags"><span class="lab">{label}</span><time>{date}</time></div>
    <h1>{title}</h1>
    {media}
    <div class="txt"><p>{text}</p>{l}</div>
  </div>
</section>
"""
